...
from .single_stage_feature_decoupling import SingleStageDetectorForFeaDecouple


@DETECTORS.register_module()
class CenterNet_Decouple(SingleStageDetectorForFeaDecouple):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CenterNet_Decouple, self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
            ...
            for ind, flip_ind in zip(img_inds[0::2], img_inds[1::2]):
                ...
                assert len(center_heatmap_preds) == len(wh_preds) == len(
                    offset_preds)
                # print(center_heatmap_preds[0].shape, offset_preds[0].shape)
                # Feature map averaging
                for i in range(len(center_heatmap_preds)):
                    center_heatmap_preds[i] = (
                        center_heatmap_preds[i][0:1] +
                        flip_tensor(center_heatmap_preds[i][1:2], flip_direction)) / 2
                    wh_preds[i] = (wh_preds[i][0:1] +
                                flip_tensor(wh_preds[i][1:2], flip_direction)) / 2
                    offset_preds[i] = offset_preds[i][0:1]
                # print(center_heatmap_preds[0].shape, wh_preds[0].shape, offset_preds[0].shape)
                bbox_list = self.bbox_head.get_bboxes(
                    center_heatmap_preds,
                    wh_preds, 
                    offset_preds,
    ...
    #             img_pair = torch.cat([imgs[ind], imgs[flip_ind]])
    #             # print(img_pair.shape)
    #             x1 = self.extract_feat(imgs[ind])
    #             x2 = self.extract_feat(imgs[flip_ind])

    #             # x = self.extract_feat(img_pair)
    #             # print(x[1].shape)
    #             center_heatmap_preds1, wh_preds1, offset_preds1 = self.bbox_head(x1)
    #             center_heatmap_preds2, wh_preds2, offset_preds2 = self.bbox_head(x2)

    #             # assert len(center_heatmap_preds) == len(wh_preds) == len(
    #             #     offset_preds)
    #             # print(center_heatmap_preds[0].shape, offset_preds[0].shape)
    #             # Feature map averaging
    #             for i in range(len(center_heatmap_preds1)):
    #                 center_heatmap_preds1[i] = (
    #                     center_heatmap_preds1[i] +
    #                     flip_tensor(center_heatmap_preds2[i], flip_direction)) / 2
    #                 wh_preds1[i] = (wh_preds1[i] +
    #                             flip_tensor(wh_preds2[i], flip_direction)) / 2
    #                 # offset_preds1[i] = offset_preds1[i]
    #             # print(center_heatmap_preds[0].shape, wh_preds[0].shape, offset_preds[0].shape)
    #             bbox_list = self.bbox_head.get_bboxes(
    #                 center_heatmap_preds1,
    #                 wh_preds1, 
    #                 offset_preds1,
    #                 img_metas[ind],
    #                 rescale=rescale,
