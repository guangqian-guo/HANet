...
class CenterNetHead(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self,
        ...
        assert len(center_heatmap_preds) == len(wh_preds) == len(
            offset_preds) == 1
        # print(len(center_heatmap_preds),center_heatmap_preds[0].shape)  # [1, 1, 144, 120]
        ...
        for batch_id in range(bs):
            ...
            for j, ct in enumerate(gt_centers):
                ...
                radius = gaussian_radius([scale_box_h, scale_box_w],
                                         min_overlap=0.3)
                # print(radius,int(radius),bbox_siz`e)
                radius = max(0, int(radius))
                ind = gt_label[j]
                gen_gaussian_target(center_heatmap_target[batch_id, ind],
                                    [ctx_int, cty_int], radius)
                # print('ctyint:', cty_int, 'ctxint:', ctx_int)
                # print('cty:', cty, 'ctx', ctx)
                # print('radius', radius)
