...
class Bottleneck(BaseModule):
    ...
    def forward(self, x, ind):
        ...
        out = _inner_forward(x, ind)

        out = self.relu(out)
        # out = torch.sigmoid(torch.mean(out.squeeze(),dim=0))
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        for i in range(num_stages):
            ...
            local_attention = nn.ModuleList([Bottleneck(in_dims[i],embed_dims[i]),
                                             Bottleneck(embed_dims[i],embed_dims[i]),
                                             Bottleneck(embed_dims[i], embed_dims[i],downsample=nn.MaxPool2d(kernel_size=3, stride=2, padding=1))])
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            local_attentions = getattr(self,f"local_attention{i+1}")
            # print(local_attention)
            # assert 1==0
            for local_attention in local_attentions:
                local_x = local_attention(local_x, i)
            ...
            # print(x.shape)
            # x += local_x
            local_attention = torch.sigmoid(torch.mean(local_x.squeeze(), dim=0))

            x = torch.mul(x, local_attention)
            outs.append(x)
...
#
@BACKBONES.register_module()
class conv_enhance_pvt4_medium(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(conv_enhance_pvt4_medium, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
...
@BACKBONES.register_module()
class conv_enhance_pvt4_large(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(conv_enhance_pvt4_large, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
