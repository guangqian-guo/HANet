...
class ConvBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.dwconv1 = DWConv(dim)
        self.dwconv2 = DWConv(dim)
    def forward(self, x, H, W):
        local_feature = self.dwconv1(x, H, W)
        x = x + self.drop_path(self.attn(self.norm1(x), H, W)+local_feature)
        local_feature = self.dwconv2(x, H, W)
        x = x + self.drop_path(self.mlp(self.norm2(x))+local_feature)
...
@BACKBONES.register_module()
class conv_enhance_pvt6_medium(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(conv_enhance_pvt6_medium, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
