...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=32, num_classes=1000, embed_dims=[64, 128, 320, 512],   #inchannels has been changed
        ...
        #== added by guo
        # conv_layer = nn.ModuleList([Bottleneck(3, 32), Bottleneck(32, 64),
        #                             Bottleneck(64, 32, downsample=nn.MaxPool2d(kernel_size=3, stride=2, padding=1))])
        conv_layer = nn.ModuleList([Bottleneck(3, 32),
                                    Bottleneck(32, 32, downsample=nn.MaxPool2d(kernel_size=3, stride=2, padding=1))])

        setattr(self,f"conv_layer", conv_layer)
        #==

        for i in range(num_stages):
            patch_embed = PatchEmbed(img_size=img_size // (2 ** (i + 1)),                        #changed by guo
                                     patch_size= 2,                                              #changed by guo
                                     in_chans=in_chans if i == 0 else embed_dims[i - 1],
                                     embed_dim=embed_dims[i])
            num_patches = patch_embed.num_patches if i != num_stages - 1 else patch_embed.num_patches + 1
            pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dims[i]))
            pos_drop = nn.Dropout(p=drop_rate)
            in_dims = [64, 64, 128, 320, 512]
            ...
            ## added by guo
            # if i >= 2:
            #     local_attention = Bottleneck(in_dims[i],embed_dims[i],downsample=nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
            #     setattr(self, f"local_attention{i+1}", local_attention)
    ...
    def _init_weights(self, m):
        ...
        # else:
        #     print(m)
            # assert 1==0, print("existing param is not initted")
    ...
    def forward_features(self, x):
        ...
        #== added by guo
        # print(x.shape)
        conv_layers = getattr(self, f"conv_layer")
...
#
@BACKBONES.register_module()
class conv_enhance_pvt2_medium(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(conv_enhance_pvt2_medium, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
...
#
# @BACKBONES.register_module()
# class conv_enhance_pvt_large(PyramidVisionTransformer):
#     def __init__(self, **kwargs):
#         super(conv_enhance_pvt_large, self).__init__(
#             patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
