...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        self.local_attention = Bottleneck(64, 64,
                                     downsample=nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
    ...
    def forward_features(self, x):
        ...
        # print(local_x.shape)
        local_x = self.local_attention(local_x)
        ...
        for i in range(self.num_stages):
            ...
            # print(x.shape)
            # x += local_x
            # print(x.shape, local_x.shape)
            if x.shape[-2:] == local_x.shape:
                x = torch.mul(x, local_x)
            else:
                local_x = F.max_pool2d(local_x.unsqueeze(0).unsqueeze(0),kernel_size=3, stride=2, padding=1).squeeze()
                x = torch.mul(x,local_x)
            outs.append(x)
...
#
@BACKBONES.register_module()
class conv_enhance_pvt3_medium(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(conv_enhance_pvt3_medium, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
