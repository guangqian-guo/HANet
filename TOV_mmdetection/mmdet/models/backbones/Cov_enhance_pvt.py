...
class Bottleneck(BaseModule):
    ...
    def forward(self, x, ind):
        ...
        def _inner_forward(x, ind):
            ...
            if self.downsample is not None and ind != 0:
...
class PyramidVisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            local_attention = getattr(self, f"local_attention{i+1}")
            # added by guo
            # if i >= 2:
            #     local_attention = getattr(self,f"local_attention{i+1}")
            #     local_x = local_attention(x, i)
            # else:
            #     local_x = 1
...
#
@BACKBONES.register_module()
class conv_enhance_pvt_medium(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(conv_enhance_pvt_medium, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
